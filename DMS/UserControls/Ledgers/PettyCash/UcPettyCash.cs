...
namespace DMS
{
    public partial class UcPettyCash : LedgerGridBase
    {
        long documentItemID = 0;
        tblPettyCashController entCntrl = new tblPettyCashController();
        ...
        public UcPettyCash(long _docItemID)
        {
            InitializeComponent();
            this.Caption = "Petty Cash List";
            this.crudMessage = new CustomMessages("Petty Cash");
        ...
        void RefreshGrid()
        {
            List<tblPettyCash> lst = entCntrl.FetchAllByDocumentItemID(documentItemID);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (grdItems.SelectedRows.Count > 0)
            {
                ...
                {
                    tblPettyCash itm = (tblPettyCash)grdItems.SelectedRows[0].DataBoundItem;
        ...
        void ApplyCalculation(tblPettyCash itm)
        {
            tblVATRate vrate = lstVat.FirstOrDefault(x => x.ID == itm.VATRateID);
            if (vrate != null && itm.GrossTaking != null)
            {
        ...
        private void grdItems_UserDeletingRow(object sender, Telerik.WinControls.UI.GridViewRowCancelEventArgs e)
        {
            ...
            if (DisplayManager.DisplayMessage(CustomMessages.DeleteConfirmation, MessageType.Confirmation) == DialogResult.Yes)
            {
                tblPettyCash itm = GetItemFromDB(e.Rows[0]);
                if (entCntrl.Delete(itm.ID))
                {
                    DisplayManager.DisplayCrudMessage(CrudMessageType.EntityDeleted, crudMessage);
                    RefreshGrid();
        ...
        private void chkVAT_ToggleStateChanged(object sender, StateChangedEventArgs args)
        {
            ...
            {
                List<tblPettyCash> lst = entCntrl.FetchNonZeroVat(documentItemID, zeroVat.ID);
