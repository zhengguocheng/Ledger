...
{
    public class JournalsGrid : ExcelBase
    {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    foreach (LedgerColumn legCol in colList)
                    {
                        ...
                        if (val != DBNull.Value)
                        {
                            ...
                            if (legCol.LedgerType == EnumLedgetType.DebitNominalCodeID || legCol.LedgerType == EnumLedgetType.CreditNominalCodeID)
                            {
        ...
        public tblJournal sGetDataItem(int row)
        {
            DataRow dr = sGetDataRow(row);
            int? id = sGetID(row);
            tblJournal ent = (id.HasValue) ? cnt.Find(id.Value) : new tblJournal();
            if (ent == null)
                ent = new tblJournal();

            DataTableMapper.SetItemFromRow<tblJournal>(ent, dr);
            return ent;
        }
        ...
        void sPopulateAutoValues(ReoGridPos pos)//On new line populate auto values
        {
            ...
            if (data == null || string.IsNullOrWhiteSpace(data.ToString()))
            {
                ...
                //if (col.LedgerType == EnumLedgetType.Date || col.LedgerType == EnumLedgetType.Reference ||
                //    col.LedgerType == EnumLedgetType.Details || col.LedgerType == EnumLedgetType.CodeCr ||
                //    col.LedgerType == EnumLedgetType.CodeDr)
                if (col.LedgerType == EnumLedgetType.Date || col.LedgerType == EnumLedgetType.Reference ||
                    col.LedgerType == EnumLedgetType.Description || col.LedgerType == EnumLedgetType.CreditNominalCodeID ||
                    col.LedgerType == EnumLedgetType.DebitNominalCodeID)
                //end modify
                {
        ...
        void CurrentWorksheet_BeforeCellEdit(object sender, unvell.ReoGrid.Events.CellBeforeEditEventArgs e)
        {
            ...
            if (col != null && (col.LedgerType == EnumLedgetType.CreditNominalCodeID || col.LedgerType == EnumLedgetType.DebitNominalCodeID))
            {
                e.IsCancelled = true;
